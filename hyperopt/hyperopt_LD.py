...
def main():
    ...
    # using fold 1 for HPO
    dataframe = pd.read_pickle('chloroform_CV_dataset_optical_rotation_5_rdkit_MOL_ee_95_MW_564_150380.pkl')
    train_val_dataframe = dataframe[(dataframe.test_fold != 1)]
    
    train_dataframe = train_val_dataframe[~train_val_dataframe.isVal_fold_1].sort_values(['SMILES_nostereo', 'ID']).reset_index(drop = True)
    val_dataframe = train_val_dataframe[train_val_dataframe.isVal_fold_1].sort_values(['SMILES_nostereo', 'ID']).reset_index(drop = True)
